  ...
  methods: {
    ...
    tagList: function () {
      ...
      getTagList(tagParams).then(response => {
        ...
        this.tagData = resp.data.records;
        this.tagOptions = resp.data.records;
      });
    },
    blogSortList: function () {
      ...
      getBlogSortList(blogSortParams).then(response => {
        if (response.data.code == this.$ECode.SUCCESS) {
          let resp  = response.data;
          this.blogSortData= resp.data.records;
          console.log(JSON.stringify(this.blogSortData));
          this.sortOptions = resp.data.records;
        }
      });
    },
    ...
    getDictList: function () {
      ...
      getListByDictTypeList(dictTypeList).then(response => {
        if (response.data.code == this.$ECode.SUCCESS) {
          console.log("字典值->",response.data.code);
          let dictMap = response.data.data;
          this.blogOriginalDictList = dictMap.sys_original_status.dictValueList;
          this.blogPublishDictList = dictMap.sys_publish_status.dictValueList;
          this.blogLevelDictList = dictMap.sys_recommend_level.dictValueList;
          this.openDictList = dictMap.sys_normal_disable.dictValueList;
          this.blogTypeDictList = dictMap.sys_blog_type.dictValueList;
    ...
    sortRemoteMethod: function (query) {
      if (query !== "") {
        var params = {};
        params.sortName = query;
        params.pageSize = 10;
        params.currentPage = 1;
        getBlogSortList(params).then(response => {
          this.sortOptions = response.data.data.records;
        });
      } else {
        this.sortOptions = [];
      }
