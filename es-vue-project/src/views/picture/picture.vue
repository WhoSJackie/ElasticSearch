      ...
      <el-row>
        ...
        >
          ...
          >
            ...
            <el-image
              :src="prePicUrl+picture.pictureUrl"
              style="cursor:pointer"
              fit="scale-down"
              @click="showPicture(prePicUrl+picture.pictureUrl)"
            />
            <div @click="showPicture(prePicUrl+picture.pictureUrl)">
...
export default {
  ...
  created() {
    ...
    //图片上传地址
    this.uploadPictureHost = process.env.WEB_API + "/pic/pictures";
    //其它数据
    this.otherData = {
      source: "picture",
  ...
  methods: {
    initPictureSortList: function (){
      ...
      if (!this.havePictureSorts) {
        ...
        getPictureSortList(params).then(function(response) {
          ...
          if (resp.code == that.$ECode.SUCCESS) {
            let pictureSorts = resp.data.records;
            that.pictureSorts = pictureSorts;
            if (pictureSorts.length > 0) {
              // 判断是否通过图片分类跳转的
              let pictureSortUid = that.pictureSortUid
    ...
    handleCurrentChange: function(val) {
      let that = this
      let pictureSort = this.pictureSort;
      let params = {}
    ...
    handleDeleteBatch: function() {
      ...
      this.$confirm("是否删除选中图片？, 是否继续?", "提示", {
        ...
        .then(() => {
          let delPic = this.pictureUids.join(","); //将数组变成,组成
          deletePictures(delPic).then(response => {
            let resp = response.data;
    ...
    fileSuccess: function(response, file, fileList) {
      ...
      if (response.code == this.$ECode.SUCCESS) {
        let succFile = response.data.succFileList;
        for (let index = 0; index < succFile.length; index++) {
          let picture = {};
          picture.fileUid = succFile[index].uid;
          picture.pictureSortUid = this.pictureSortUid
          picture.picName = succFile[index].picName
          this.pictureUploadList.push(picture)
        }

        // this.count = this.count + 1;
        if(succFile.length == fileList.length) {
          addPictures(this.pictureUploadList).then(res => {
            let resp = res.data;
            if (resp.code == this.$ECode.SUCCESS) {
              this.$commonUtil.message.success(resp.msg)
              this.handleCurrentChange(this.currentPage);
        ...
        this.$commonUtil.message.error(response.msg)
      }
    }
  }
};
